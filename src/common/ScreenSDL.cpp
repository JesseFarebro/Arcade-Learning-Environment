...
ScreenSDL::ScreenSDL(OSystem* osystem) : Screen(osystem), mediaSource(NULL), sound(NULL), colourPalette(NULL) {
  ...
  renderer = SDL_CreateRenderer(window, -1, rendererInfo.flags | SDL_RENDERER_PRESENTVSYNC);
  if (renderer == nullptr) {
...
int ScreenSDL::getScaleFactor() {
  ...
  if (widthHeuristic > heightHeuristic) {
    scaleFactor = round(static_cast<float>(widthHeuristic) / static_cast<float>(windowWidth));
  } else {
    scaleFactor = round(static_cast<float>(heightHeuristic) / static_cast<float>(windowHeight));
  }
  ...
  switch(event.type) {
    ...
    case SDL_KEYDOWN:
      switch(event.key.keysym.sym) {
        case SDLK_LEFT:
          FPS = max(FPS - 5, static_cast<uint32_t>(5));
          sound->setFrameRate(FPS);
          break;
        case SDLK_RIGHT:
          FPS = min(this->maxFPS, FPS + 5);
          sound->setFrameRate(FPS);
          break;
        case SDLK_DOWN:
          for (int i = 0; i < 5; i++)
