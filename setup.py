...
class CMakeBuild(build_ext):
    def build_extensions(self):
        ...
        for ext in self.extensions:
            ...
            # we need to specify --config Release at build type
...
def _parse_version(filename):
    """
    Parses ALEVERSION from `CMakeLists.txt`
    ...
    returns:
        version:
            ...
            2) Running in CI on master will be of the form VERSION.dev[#commits since last tag]
            3) Running in CI with a version tag will be of the form TAGGED_VERSION
    raises:
        RuntimeError:
    ...
    if os.environ.get(ref, False):
        # If the git ref is a tag verify the tag and don't use a suffix
        # If the git ref isn't a tag we'll use the # of commits since the last tag as a suffix
        if re.match(tag_regex, os.environ.get(ref)):
            ...
            description = subprocess.check_output(["git", "describe", "--tags"])
            version_tag, commits = re.findall(r"-?(\d.*?)-", str(description))
            assert version_tag == version, (
                "Tagged version (%s) seems to be different than parsed version (%s)"
                % (version_tag, version)
            )
            version_suffix += commits
